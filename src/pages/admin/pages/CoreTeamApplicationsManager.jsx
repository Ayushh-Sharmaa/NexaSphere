    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          background: 'linear-gradient(135deg,#7b6fff,#CC1111)',
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--text2)', display: 'flex', flexWrap: 'wrap', gap: '4px 12px' }}>
            {app.role && <span style={{ color: 'var(--red)', fontWeight: 600 }}>{app.role}</span>}
            <span>{app.branch || app.year}</span>
            {app.year && app.branch && <span>{app.year}</span>}
          </div>
        </div>
      ...
      {expanded && (
        <div style={{ padding: '0 16px 16px', borderTop: '1px solid var(--border)' }}>
          <div style={{ paddingTop: 12, display: 'grid', gap: 0 }}>
            ...
            <DetailRow label="Year" value={app.year} />
            <DetailRow label="Branch" value={app.branch} />
            <DetailRow label="Section" value={app.section} />
            <DetailRow label="Role Applied" value={app.role} />
            <DetailRow label="Interests" value={app.interests} />
            <DetailRow label="Skills" value={app.skills} />
            <DetailRow label="Communication" value={app.comms} />
            <DetailRow label="Campus Exp." value={app.campusExp} />
            <DetailRow label="Campus Details" value={app.campusExpDetails} />
            <DetailRow label="Commit Hours" value={app.commitHours} />
            <DetailRow label="Attend Campus" value={app.attendCampus} />
            <DetailRow label="Assessment OK" value={app.assessmentOk} />
            <DetailRow label="GitHub" value={app.links} />
            <DetailRow label="Why Join" value={app.whyJoin} />
            <DetailRow label="Anything Else" value={app.anythingElse} />
...
export function CoreTeamApplicationsManager() {
  ...
  const loadApps = useCallback(async () => {
    setLoading(true);
    try {
      const data = await api.coreTeamApps.getAll();
      setApps(data?.apps || []);
    } catch {
      setApps([]);
    } finally {
  ...
  const filtered = apps.filter(a => {
    ...
    if (search) {
      ...
      return (
        ...
        (a.interests || '').toLowerCase().includes(q)
      );
    }
    return true;
  });
  ...
  return (
    <div className="page">
      <div className="page-header">
        <h2 className="page-title">Core Team Applications</h2>
      ...
      <div style={{ position: 'relative', marginBottom: 16 }}>
        ...
        <input
          ...
          placeholder="Search by name, email, role, branch, interests..."
          style={{
      ...
      {!loading && filtered.length === 0 && (
        <div className="empty-state">
          {apps.length === 0
            ? 'No core team applications yet. When users submit the recruitment form, they will appear here.'
