...
export const searchController = {
  async search(req, res) {
    try {
      ...
      const { isTypesenseEnabled, typesenseClient } =
        await import("../config/typesense.js");
      ...
      if (isTypesenseEnabled) {
        try {
            ...
            hits.forEach((hit) => {
              ...
              if (collectionName === "events") {
                ...
                results.push({
                  ...
                  description:
                    getHighlight("role", doc.role) ||
                    getHighlight("bio", doc.bio),
                  tags: doc.skills,
                  url: `/team`,
                  score: hit.text_match || 0,
                });
          ...
          console.error(
            "Typesense search failed, falling back to local search:",
            typesenseErr
          );
        }
      }
      ...
      if (type === "all" || type === "users" || type === "portfolios") {
        try {
          ...
          if (!allPortfolios || allPortfolios.length === 0) {
            const PORTFOLIOS_FILE = path.join(
              __dirname,
              "..",
              "data",
              "portfolios.json"
          ...
          if (!allCats || allCats.length === 0) {
            allCats = [
              ...
              },
              {
                ...
                slug: "events",
                description:
                  "Questions and discussions about past and upcoming events",
                icon: "📅",
              },
              {
                id: 3,
                ...
                slug: "technical-help",
                description:
                  "Get help with technical issues, code, and projects",
                icon: "💻",
              },
              {
                id: 4,
      ...
      if (type === "all" || type === "posts" || type === "discussions") {
        try {
          const threadsRes = await forumRepository.listThreads({
            q: query,
            limit,
          });
      ...
      if (type === "all" || type === "resources") {
        try {
          const resourcesRes = await resourcesRepository.list({
            q: query,
            limit,
          });
      ...
      return sendError(req, res, "Search failed", 500, "INTERNAL_ERROR", {
        results: [],
        total: 0,
      });
    }
  ...
  async trending(req, res) {
    try {
      const popularSearches =
        await searchAnalyticsRepository.getPopularSearches(5);
      const events = await eventsRepository.list({ page: 1, limit: 100 });
      ...
      const sorted = allEvents
        ...
        .map((ev) => ({
          ...
          tags: ev.tags,
          registrationCount:
            ev.registrationCount || ev.registrations?.length || 0,
          image: ev.image,
          status: ev.status,
          url: `/events/${ev.id}`,
        }));
    ...
    try {
      ...
      if (eventId) {
          ...
          const scored = allEvents
            ...
            .map((ev) => {
              const evTags = new Set(
                (ev.tags || []).map((t) => t.toLowerCase())
              );
              const intersection = [...targetTags].filter((t) =>
                evTags.has(t)
              ).length;
        ...
        recommended = allEvents
          ...
          .sort((a, b) => {
            const aScore =
              (a.registrationCount || 0) * 0.4 +
              (new Date(a.date) > now ? 0.3 : 0);
            const bScore =
              (b.registrationCount || 0) * 0.4 +
              (new Date(b.date) > now ? 0.3 : 0);
          ...
          .map((ev) => ({
            ...
            tags: ev.tags,
            score:
              (ev.registrationCount || 0) * 0.4 +
              (new Date(ev.date) > now ? 0.3 : 0),
            url: `/events/${ev.id}`,
          }));
      }
