...
export const cancelRegistration = wrapAsync(async (req, res) => {
  ...
  if (authenticatedEmail !== sanitizedEmail) {
    return res
      .status(403)
      .json({ error: "Forbidden: you can only cancel your own registration" });
  }
  ...
  const registration = await registrationsRepository.findByEmailAndEvent(
    sanitizedEmail,
    eventId
  );
  if (!registration || registration.status !== "confirmed") {
    return res
      .status(404)
      .json({ error: "No confirmed registration found for this email" });
  }
  ...
  if (promoted) {
    try {
      ...
      console.error(
        "[EventRegistration] Failed to broadcast or schedule promotion:",
        realtimeErr
      );
    }
  }
