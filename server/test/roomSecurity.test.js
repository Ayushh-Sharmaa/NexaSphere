...
test("Security Audit & Validation: Socket Room Hardening", async (t) => {
  ...
  await t.test(
    "Scenario 2: Unapproved and arbitrary rooms are rejected",
    () => {
  ...
  await t.test(
    "Scenario 4: Protected room joins reject unauthorized sockets",
    () => {
  ...
  await t.test(
    "Scenario 5: Workspace dynamic room joins require structured ID validation",
    () => {
      ...
      socket.emit("join_room", "workspace; DROP TABLE users; --", {
        name: "Alice",
      });
  ...
  await t.test(
    "Scenario 6: Per-socket room limits prevent unbounded room subscriptions",
    () => {
  ...
  await t.test(
    ...
    () => {
      ...
      const aliceUpdates = alice.emittedTo.filter(
        (e) => e.event === "workspace_update"
      );
      ...
      const eveUpdates = eve.emittedTo.filter(
        (e) => e.event === "workspace_update"
      );
      ...
      const aliceDocs = alice.emittedTo.filter(
        (e) => e.event === "document_change"
      );
      assert.equal(aliceDocs.length, 1);

      eve.emit("document_change", { roomId: room, doc: "doc1" });
      const eveDocs = eve.emittedTo.filter(
        (e) => e.event === "document_change"
      );
      ...
      const aliceCursors = alice.emittedTo.filter(
        (e) => e.event === "cursor_moved"
      );
      assert.equal(aliceCursors.length, 1);

      eve.emit("cursor_moved", { roomId: room, pos: 99 });
      const eveCursors = eve.emittedTo.filter(
        (e) => e.event === "cursor_moved"
      );
  ...
  await t.test(
    "Scenario 8: Workspace event relay blocked when roomId is missing",
    () => {
  ...
  await t.test(
    "Scenario 9: leave_room clears workspace membership tracking",
    () => {
      ...
      const updates = socket.emittedTo.filter(
        (e) => e.event === "workspace_update"
      );
      assert.equal(updates.length, 0);

      socket.emit("document_change", { roomId: room, doc: "doc2" });
      const docs = socket.emittedTo.filter(
        (e) => e.event === "document_change"
      );
  ...
  await t.test(
    "Scenario 10: disconnect cleans up workspace membership tracking",
    () => {
  ...
  await t.test(
    "Scenario 11: join_room rate limiting prevents room enumeration",
    async () => {
      ...
      assert.equal(
        joinedCount,
        0,
        `Expected 0 joined in round 3, got ${joinedCount}`
      );
    }
  );
});
