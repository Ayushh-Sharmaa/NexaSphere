...
function mapMentorRow(row) {
  return {
    ...
    email: row.email,
    domains:
      typeof row.domains === "string" ? JSON.parse(row.domains) : row.domains,
    bio: row.bio,
...
export const mentorshipRepository = {
  ...
  async listMentorships({ page = 1, limit = 20, status, email } = {}) {
    ...
    return withDb(async (client) => {
      ...
      return {
        rows: rows.map(mapMentorshipRow),
        total: countResult.rows[0]?.total ?? 0,
      };
    });
  },
  ...
  async listBuddyPairs({ page = 1, limit = 50, email } = {}) {
    ...
    return withDb(async (client) => {
      ...
      if (email) {
        conditions.push(
          `(buddy1_email = $${paramIdx} or buddy2_email = $${paramIdx})`
        );
        params.push(email);
        paramIdx++;
      }
