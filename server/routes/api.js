import workflowAutomationRoutes from "./workflowAutomation.js";
import digitalAssetRoutes from "./digitalAsset.js";
import knowledgeAssistantRoutes from "./knowledgeAssistant.js";
import reportingCenterRoutes from "./reportingCenter.js";
import budgetRoutes from "./budget.js";
import resourceDiscoveryRoutes from "./resourceDiscovery.js";
import notificationCampaignRoutes from "./notificationCampaign.js";
...
router.get(
  "/api/content/events/:eventId/seats",
  venueLayoutsController.getEventSeatMap
);
router.post(
...
router.post(
  "/api/auth/login",
  authRateLimiter,
  authRefreshController.localLogin
);
// Rotate a refresh token → new access token + new refresh token
router.post(
  "/api/auth/refresh",
  authRateLimiter,
  authRefreshController.refreshTokens
);
...
router.post(
  "/api/auth/logout-all",
  requireStudentAuth,
  authRefreshController.logoutAll
);
// List active sessions for device management UI
router.get(
  "/api/auth/sessions",
  requireStudentAuth,
  authRefreshController.listSessions
);
router.post(
...
router.get(
  "/api/admin/2fa/settings/status",
  adminAuthMiddleware.requireAdmin,
  adminAuthMiddleware.getTwoFactorStatus
);
router.post(
  "/api/admin/2fa/settings/setup/init",
  adminAuthMiddleware.requireAdmin,
  adminAuthMiddleware.initTwoFactorSetup
);
router.post(
  "/api/admin/2fa/settings/setup/verify",
  adminAuthMiddleware.requireAdmin,
  adminAuthMiddleware.verifySettingsTwoFactorSetup
);
router.post(
  "/api/admin/2fa/settings/disable",
  adminAuthMiddleware.requireAdmin,
  adminAuthMiddleware.disableTwoFactor
);
...
// Get follow counts
router.get(
  "/api/student/users/:userId/follow-counts",
  followsController.getFollowCounts
);
