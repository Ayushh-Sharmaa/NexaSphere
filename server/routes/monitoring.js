...
router.get("/status-history", async (req, res) => {
  try {
    ...
    const downtimeEventsCount = incidents.filter(
      (i) => i.status !== "resolved"
    ).length;
...
router.get("/errors/endpoint", requireMonitoringAuth, (req, res) => {
  try {
    ...
    sendError(
      req,
      res,
      "Failed to fetch endpoint errors",
      500,
      "INTERNAL_ERROR"
    );
  }
...
router.get("/incident-alerts", requireMonitoringAuth, (req, res) => {
  try {
    ...
    sendError(
      req,
      res,
      "Failed to fetch incident alerts",
      500,
      "INTERNAL_ERROR"
    );
  }
...
router.get("/incidents", requireMonitoringAuth, (req, res) => {
  try {
    ...
    sendError(
      req,
      res,
      "Failed to fetch incident information",
      500,
      "INTERNAL_ERROR"
    );
  }
...
router.get("/session-security", requireMonitoringAuth, (req, res) => {
  try {
    ...
    sendError(
      req,
      res,
      "Failed to fetch session security data",
      500,
      "INTERNAL_ERROR"
    );
  }
...
router.get("/failover-status", requireMonitoringAuth, async (req, res) => {
  ...
  const isDrConfigured =
    process.env.DATABASE_URL_REPLICA && process.env.REDIS_URL_SECONDARY; // Example check
