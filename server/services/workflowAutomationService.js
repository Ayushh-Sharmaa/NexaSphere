...
export function createWorkflow(data) {
  const nextId =
    workflows.length > 0 ? Math.max(...workflows.map((w) => w.id)) + 1 : 1;
  const workflow = {
    id: nextId,
    ...data,
    status: "Active",
...
export function submitRequest(data) {
  const nextId =
    requests.length > 0 ? Math.max(...requests.map((r) => r.id)) + 1 : 101;
  const request = {
    id: nextId,
...
export function approveRequest(id, approver = "Admin") {
...
export function rejectRequest(id, approver = "Admin") {
  const request = requests.find((r) => r.id == id);
...
export function escalatePendingRequests() {
