...
function Sparkline({ data, color = "#6366f1" }) {
  if (!data || data.length === 0)
    return (
      <span style={{ color: "#cbd5e1", fontSize: "0.75rem" }}>No data</span>
    );
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: "0.8rem", color: "#94a3b8" }}>
        {plural((definition.properties || []).length, "property")} &bull;
        Created by <strong>{definition.created_by}</strong> &bull;{" "}
  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    try {
      const [analyticsRes, logsRes] = await Promise.all([
        fetch(
          `${API_BASE}/api/admin/custom-events/definitions/${definition.id}/analytics?days=${days}`,
          { credentials: "include" }
        ),
        fetch(
    ...
    <div>
      ...
      {loading ? (
        ...
        <>
          ...
          >
            {[
              ...
              },
              {
                ...
                value: analytics?.allTimeTotal ?? 0,
                icon: "📈",
              },
              {
                label: "Unique Users",
          ...
          >
            <h4
              style={{ margin: "0 0 16px", color: "#1e293b", fontWeight: 600 }}
            >
              Daily Occurrences (Last {days} days)
            </h4>
...
export function CustomEventTracking() {
  ...
  async function handleDelete(id) {
    if (
      !window.confirm(
        "Delete this event definition and all its logs? This cannot be undone."
      )
    )
      return;
    ...
    <div style={pageStyle}>
      ...
      >
        <div>
          ...
          >
            Define custom events, track their occurrences, and export analytics
            — no code required.
          </p>
        </div>
      ...
      {showForm && (
        <div style={overlayStyle}>
          ...
          >
            <h3
              style={{ margin: "0 0 20px", color: "#1e293b", fontWeight: 700 }}
            >
      ...
      {loading ? (
        ...
        >
          ...
          <p
            style={{ fontWeight: 600, color: "#64748b", marginBottom: "16px" }}
          >
