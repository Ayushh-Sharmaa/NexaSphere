...
export function EventScanner() {
  ...
  const syncPendingCheckins = async () => {
    ...
    try {
      ...
      eventEmitter.emit(EVENTS.NOTIFY, {
        type: "error",
        message: "Failed to sync check-ins",
      });
    } finally {
      setIsSyncing(false);
    }
  ...
  const markAttendance = useCallback(
    async (payload) => {
      ...
      try {
        let result;
        if (isOfflineMode) {
          result = api.eventRegistrations.markOfflineAttendance(
            selectedEventId,
            payload.email
          );
          loadOfflineState();
        } else {
          result = await api.eventRegistrations.markAttendance(
            selectedEventId,
            {
              ...payload,
  ...
  const handleManualSubmit = (e) => {
    ...
    if (isOfflineMode) {
      ...
      markAttendance({
        email: (manualEmail || searchTerm).trim().toLowerCase(),
      });
    } else {
    ...
    <div className="page">
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <label
            style={{ fontSize: "0.9rem", color: "var(--admin-text-muted)" }}
          >
            Offline Mode
          </label>
          <div
      ...
      {selectedEventId && (
        <>
          ...
          {!isOfflineMode && (
            ...
            >
              ...
              >
                ...
                {isDownloading
                  ? "Downloading..."
                  : "Download Attendee List for Offline Use"}
              </button>
          ...
          >
            ...
            >
              ...
              <button
                type="submit"
                disabled={
                  scanning || (isOfflineMode && !manualEmail && !searchTerm)
                }
          ...
          {attendanceLog.length > 0 && (
            <div>
              <h3
                style={{ marginBottom: 12, fontFamily: "Rajdhani,sans-serif" }}
              >
