...
const MAIN_APP =
  import.meta.env.VITE_MAIN_APP_URL || "https://nexasphere-glbajaj.vercel.app";
    ...
    if (key === "resources") {
      const initialResources = [
        ...
        },
        {
          ...
          description:
            "Ready-to-use MERN stack project template with auth and CRUD.",
          fileUrl: "#",
      ...
      const initialSponsors = [
        {
          ...
          logoUrl: "",
          description:
            "Leading technology solutions provider supporting innovation in education.",
        ...
        },
        {
          ...
          logoUrl: "",
          description:
            "Software development consultancy fostering campus tech communities.",
...
function notifyContentUpdated(key) {
  ...
  const bridge = document.querySelector(
    'iframe[title="NexaSphere Sync Bridge"]'
  );
      ...
      // /api/admin/reports/engagement
        ...
        const seedUsers = Array.from({ length: 45 }, (_, i) => {
          ...
          const engagementScore = Math.round(
            score30 + scoreEvents + scorePortfolio
          );
...
export const api = {
  ...
  eventRegistrations: {
    ...
    analytics: (eventId) =>
      fetchWithAuth(`/api/admin/events/${eventId}/analytics`),
  },
  events: {
  ...
  activityEvents: {
    getAll: (activityKey) =>
      fetchWithAuth(`/api/admin/activity-events/${activityKey}`),
    create: async (activityKey, event) => {
      ...
      const result = await fetchWithAuth(
        `/api/admin/activity-events/${activityKey}`,
        {
          method: "POST",
          body: JSON.stringify(event),
        }
    ...
    delete: async (activityKey, eventId) => {
      ...
      await fetchWithAuth(
        `/api/admin/activity-events/${activityKey}/${eventId}`,
        {
          method: "DELETE",
        }
      );
  ...
  recruitment: {
    getAll: (params = {}) => {
      const query = new URLSearchParams(params).toString();
      return fetchWithAuth(
        `/api/admin/submissions/recruitment${query ? `?${query}` : ""}`
      );
    },
    updateStatus: async (id, status) => {
      const result = await fetchWithAuth(
        `/api/admin/submissions/recruitment/${id}/status`,
        {
          method: "PATCH",
          body: JSON.stringify({ status }),
        }
  ...
  certificates: {
    ...
    updateTemplate: async (id, template) => {
      const result = await fetchWithAuth(
        `/api/admin/certificates/templates/${id}`,
        {
          method: "PUT",
          body: JSON.stringify(template),
        }
    ...
    deleteTemplate: async (id) => {
      await fetchWithAuth(`/api/admin/certificates/templates/${id}`, {
        method: "DELETE",
      });
      eventEmitter.emit(EVENTS.NOTIFY, {
        type: "success",
    ...
    getParticipants: (eventId) =>
      fetchWithAuth(`/api/admin/certificates/participants/${eventId}`),
    ...
    bulkAddParticipants: async (eventId, participants) => {
      return fetchWithAuth(
        `/api/admin/certificates/participants/${eventId}/bulk`,
        {
          method: "POST",
  ...
  portfolios: {
    ...
    getAchievements: (username) =>
      fetchWithAuth(
        `/api/admin/portfolios/${encodeURIComponent(username)}/achievements`
      ),
  ...
  announcements: {
    ...
    delete: async (id) => {
      ...
      await fetchWithAuth(`/api/admin/announcements/${id}`, {
        method: "DELETE",
      });
  ...
  circuitBreaker: {
    ...
    reset: (name) =>
      fetchWithAuth(
        `/api/admin/circuit-breaker/reset/${encodeURIComponent(name)}`,
        {
          method: "POST",
        }
      ),
    retry: (name) =>
      fetchWithAuth(
        `/api/admin/circuit-breaker/retry/${encodeURIComponent(name)}`,
        {
          method: "POST",
        }
      ),
  ...
  waitingRoom: {
    getQueue: (eventId) =>
      fetchWithAuth(
        `/api/admin/events/${encodeURIComponent(eventId)}/waiting-room`
      ),
    admitOne: (eventId) =>
      fetchWithAuth(
        `/api/admin/events/${encodeURIComponent(eventId)}/waiting-room/admit-one`,
        {
          method: "POST",
        }
      ),
    admitAll: (eventId) =>
      fetchWithAuth(
        `/api/admin/events/${encodeURIComponent(eventId)}/waiting-room/admit-all`,
        {
          method: "POST",
        }
      ),
    ...
    sendMessage: (eventId, message) =>
      fetchWithAuth(
        `/api/admin/events/${encodeURIComponent(eventId)}/waiting-room/message`,
        {
          method: "POST",
  ...
  impersonate: {
    start: (userId) =>
      fetchWithAuth(`/api/admin/impersonate/start/${userId}`, {
        method: "POST",
      }),
    stop: () =>
      fetchWithAuth("/api/admin/impersonate/stop", { method: "POST" }),
  ...
  forum: {
    getAll: async (params = {}) => {
      ...
      return fetchWithAuth(
        `/api/admin/forum/threads${query ? `?${query}` : ""}`
      );
    },
    moderate: async (id, status) => {
      if (auth.isOfflineMode()) {
    ...
    moderateReply: async (id, status) => {
      ...
      const result = await fetchWithAuth(
        `/api/admin/forum/replies/${id}/moderate`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ status }),
  ...
  subscriptions: {
    getAll: async () => {
      if (auth.isOfflineMode()) {
        const subs = safeJsonParse(
          localStorage.getItem("ns_db_subscriptions"),
          []
        );
    ...
    getBillingHistory: (userId) =>
      fetchWithAuth(`/api/admin/subscriptions/${userId}/billing`),
  },
  ...
  backups: {
    ...
    getRestoreHistory: () =>
      fetchWithAuth("/api/admin/backups/restore-test-history"),
    runManual: (type) =>
    ...
    delete: (key) =>
      fetchWithAuth(`/api/admin/backups/${encodeURIComponent(key)}`, {
        method: "DELETE",
      }),
  },
  ...
  moderation: {
    ...
    getAppeals: (filters = {}) => {
      const query = new URLSearchParams(filters).toString();
      return fetchWithAuth(
        `/api/moderation/appeals${query ? `?${query}` : ""}`
      );
    },
